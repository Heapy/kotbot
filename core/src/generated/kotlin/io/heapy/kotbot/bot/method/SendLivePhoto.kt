...
import io.heapy.kotbot.bot.model.LivePhoto
 ...
 */
@Serializable
public data class SendLivePhoto(
    /**
    ...
    /**
     * Live photo video to send. The video must be no longer than 10 seconds and must not exceed 10 MB in size. Pass a file_id as String to send a video that exists on the Telegram servers (recommended) or upload a new video using multipart/form-data. [More information on Sending Files &raquo;](https://core.telegram.org/bots/api/#sending-files). Sending live photos by a URL is currently unsupported.
     */
    public val live_photo: LivePhoto,
    /**
     * The static photo to send. Pass a file_id as String to send a photo that exists on the Telegram servers (recommended) or upload a new video using multipart/form-data. [More information on Sending Files &raquo;](https://core.telegram.org/bots/api/#sending-files). Sending live photos by a URL is currently unsupported.
     */
...
) : Method<SendLivePhoto, Message> by Companion {
    public companion object : Method<SendLivePhoto, Message> {
        override val _deserializer: KSerializer<Response<Message>> =
                Response.serializer(Message.serializer())

        override val _serializer: KSerializer<SendLivePhoto> = serializer()

        override val _name: String = "sendLivePhoto"
    }
}
